...
app.post("/generate-tips", async (req, res) => {
  try {
    ...
    fs1.writeFileSync(
      path.join(__dirname, "public", `tip_${tipId}.json`),
      JSON.stringify(tipData)
    );

    // 🔁 Fire-and-forget audio generation
    setImmediate(() => {
      generateAudioAsync(tipsText, tipData.id);
    });
...
const generateAudioAsync = async (tip, tipId) => {
	console.log("generating")
  try {
    const audioContent = tip;
    ...
    console.error(`Error generating audio for tip ${tipId}:`, err);
  }
};
...
app.get("/audio/:tipId", async (req, res) => {
  const { tipId } = req.params;
  console.log("tipId", tipId)
//   await sleep(2000)
  const filePath = path.join(__dirname, "public", `tip_${tipId}.mp3`);
